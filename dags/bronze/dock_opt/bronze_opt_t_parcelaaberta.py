...
dataset = Dataset( 'bronze_opt_t_parcelaaberta' )

@dag(
    dag_id = 'bronze_opt_t_parcelaaberta',
    start_date = datetime(2023, 1, 10),
    tags = ['fonte_opt', 'camada_bronze', 'processo_full'],
    schedule = [Dataset('raw_opt_t_parcelaaberta')],
...
def raw_to_bronze_full():
    ...
    run_databricks_notebook = DatabricksSubmitRunOperator(
        ...
        outlets = [Dataset('raw_opt_t_parcelaaberta')],
        ...
        notebook_task = {
            ...
            'base_parameters': {
                'path': 'dock_opt/t_parcelaaberta', 
                'catalog': 'hml',
                'schema': 'bronze',
                'table': 'opt_t_parcelaaberta'
            }
        }
    )    
