...
dataset = Dataset( 'raw_opt_t_servicoconta' )

@dag(
    dag_id = 'raw_opt_t_servicoconta',
...
def raw_adf_db_to_datalake():
    ...
    wait_dock_opt = SqlSensor(
        ...
        conn_id = 'dock_opt',
        sql = 'select count(1) from t_control where name = \'' + 't_servicoconta' + '\' and convert(date, dt_start) = convert(date, getdate()) and dt_finish is not null',
        poke_interval = 60 * 5,
        mode = 'reschedule',
        timeout = 60 * 60 * 12
    )
    ...
    run_pipeline = AzureDataFactoryRunPipelineOperator(
        ...
        parameters = {
            "table_name": 't_servicoconta',
            "container_name": 'raw/dock_opt/t_servicoconta/',
            "file_name": 't_servicoconta' + '_' + '{{ ti.xcom_pull(task_ids="current_date") }}' + '.parquet'
        }
    )
