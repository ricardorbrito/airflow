...
dataset = Dataset( 'raw_opt_t_eventopagamento' )
...
def raw_adf_dock_opt_to_datalake_incremental():
    ...
    wait_dock_opt = SqlSensor(
        ...
        conn_id = 'dock_opt',
        sql = 'select count(1) from t_control where name = \'' + 't_eventopagamento' + '\' and convert(date, dt_start) = convert(date, getdate()) and dt_finish is not null',
        poke_interval = 60 * 5,
        mode = 'reschedule',
        timeout = 60 * 60 * 12
    )
    ...
    run_pipeline = AzureDataFactoryRunPipelineOperator(
        ...
        parameters = {
            'table_name': 't_eventopagamento',
            'container_name': 'raw/dock_opt/t_eventopagamento/',
            'file_name': 't_eventopagamento' + '_' + '{{ ti.xcom_pull(task_ids="get_last_date_result") }}' + '_to_' + '{{ ti.xcom_pull(task_ids="current_date") }}' + '.parquet',
            'query': 'select * from ' +  't_eventopagamento' + ' where ' +  'dt_movimento' + ' > ' + '\'{{ ti.xcom_pull(task_ids="get_last_date_result") }}\''
        }
    )
