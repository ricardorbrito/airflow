...
dataset = Dataset( 'raw_opt_t_parcelaaberta' )

@dag(
    dag_id = 'raw_opt_t_parcelaaberta',
...
def raw_adf_db_to_datalake():
    ...
    wait_dock_opt = SqlSensor(
        ...
        conn_id = 'dock_opt',
        sql = 'select count(1) from t_control where name = \'' + 't_parcelaaberta' + '\' and convert(date, dt_start) = convert(date, getdate()) and dt_finish is not null',
        poke_interval = 60 * 5,
        mode = 'reschedule',
        timeout = 60 * 60 * 12
    )
    ...
    run_pipeline = AzureDataFactoryRunPipelineOperator(
        ...
        parameters = {
            "table_name": 't_parcelaaberta',
            "container_name": 'raw/dock_opt/t_parcelaaberta/',
            "file_name": 't_parcelaaberta' + '_' + '{{ ti.xcom_pull(task_ids="current_date") }}' + '.parquet'
        }
    )
